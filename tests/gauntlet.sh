...
HOME_CYCLO_SK="$(test -e "$FS_HOME3/.cursor/skills/complexity/SKILL.md" && echo yes || echo no)"
...
run_test "install provides complexity skill" "yes" "$HOME_CYCLO_SK"
...
run_test "install copies types.mdc to user rules" "yes" "$HOME_TYPES"
run_test "install prunes alwaysApply from pack .cursor/rules" "no" "$PACK_AGENT"
run_test "install prunes types.mdc from pack .cursor/rules" "no" "$PACK_TYPES"
