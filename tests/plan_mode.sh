...
rm -rf "$PACK/state"
RESULT="$(cat "$PACK/tests/fixtures/sessionStart_plan_mode.json" | bash "$PACK/hooks/session_start.sh")"
...
PLAN_MODE_WRITTEN="$(cat "$PACK/state/mode" 2>/dev/null || echo "")"
run_test "session_start writes state/mode=plan" "plan" "$PLAN_MODE_WRITTEN"

rm -rf "$PACK/state"; mkdir -p "$PACK/state"
printf 'plan\n' > "$PACK/state/mode"
RESULT="$(cat "$PACK/tests/fixtures/beforeSubmitPrompt_plan_mode.json" | bash "$PACK/hooks/before_submit_prompt.sh")"
RESULT_IS_EMPTY="$(printf '%s' "$RESULT" | jq -r 'if . == {} then "quiet" else "scaffold" end')"
...
RESULT="$(cat "$PACK/tests/fixtures/stop_valid_intent.json" | bash "$PACK/hooks/stop_gate.sh" 2>/dev/null | jq -r 'if .followup_message then "followup" else "accept" end')"
