...
RESULT="$(cat "$PACK/tests/fixtures/beforeSubmitPrompt_code.json" | bash "$PACK/shared/hooks/before_submit_prompt.sh" | jq -r '.additionalContext | length > 0')"
run_test "before_submit_prompt emits context" "true" "$RESULT"

RESULT="$(cat "$PACK/tests/fixtures/sessionStart.json" | bash "$PACK/shared/hooks/session_start.sh" | jq -r '.additionalContext | length > 0')"
run_test "session_start emits context" "true" "$RESULT"

rm -rf "$PACK/state"
RESULT="$(cat "$PACK/tests/fixtures/preToolUse_write_small.json" | bash "$PACK/shared/hooks/lean_gate.sh" | jq -r '.action')"
...
RESULT="$(echo '{"tool_name":"Read","tool_input":{"file_path":"x"}}' | bash "$PACK/shared/hooks/pre_tool_use.sh" | jq -r 'if . == {} then "pass" else .action end')"
run_test "pre_tool_use allows Read" "pass" "$RESULT"

RESULT="$(cat "$PACK/tests/fixtures/preToolUse_bash_destructive.json" | bash "$PACK/shared/hooks/pre_tool_use.sh" | jq -r '.action')"
run_test "pre_tool_use blocks destructive root-delete" "deny" "$RESULT"

RESULT="$(echo '{"tool_name":"Bash","hook_event_name":"preToolUse","tool_input":{"command":"rm -rf /"}}' | bash "$PACK/shared/hooks/pre_tool_use.sh" | jq -r '.action // "none"')"
...
RESULT="$(cat "$PACK/tests/fixtures/preToolUse_scope_expansion.json" | bash "$PACK/shared/hooks/pre_tool_use.sh" | jq -r '.action // "allow"')"
run_test "pre_tool_use scope expansion uses allow action (was warn)" "allow" "$RESULT"

rm -rf "$PACK/state"
RESULT="$(echo '{"prompt":"implement a login form"}' | bash "$PACK/shared/hooks/before_submit_prompt.sh" | jq -r '.additionalContext | test("CONSTRAINTS:")')"
run_test "before_submit reminds constraints on unbound task" "true" "$RESULT"

rm -rf "$PACK/state"
RESULT="$(cat "$PACK/tests/fixtures/beforeSubmitPrompt_code_ponytail.json" | bash "$PACK/shared/hooks/before_submit_prompt.sh" | jq -r '.additionalContext | test("PONYTAIL LADDER")')"
run_test "before_submit injects ponytail ladder on code route" "true" "$RESULT"

rm -rf "$PACK/state"
RESULT="$(echo '{"prompt":"the form breaks on submit"}' | bash "$PACK/shared/hooks/before_submit_prompt.sh" | jq -r '.additionalContext | test("PONYTAIL LADDER")')"
run_test "before_submit injects ponytail even with no code keywords (always-code routing)" "true" "$RESULT"

rm -rf "$PACK/state"
RESULT="$(echo '{"prompt":"the form breaks on submit"}' | bash "$PACK/shared/hooks/before_submit_prompt.sh" | jq -r '.additionalContext | test("ROUTE_CLASSIFY: code")')"
...
LOC_OK=1
for f in "$PACK"/shared/hooks/session_start.sh "$PACK"/shared/hooks/session_end.sh "$PACK"/shared/hooks/before_submit_prompt.sh "$PACK"/shared/hooks/stop_gate.sh "$PACK"/shared/hooks/lean_gate.sh "$PACK"/shared/hooks/pre_tool_use.sh "$PACK"/shared/hooks/subagent_start.sh "$PACK"/shared/hooks/subagent_stop.sh "$PACK"/shared/hooks/after_shell.sh "$PACK"/shared/hooks/before_read_file.sh; do
  n="$(wc -l < "$f")"
...
RESULT="$(HERE="$PACK/shared/hooks" bash -c 'source "$0/lib/common.sh" && resolve_root && type emit_allow >/dev/null && type emit_deny >/dev/null && type emit_followup >/dev/null && type acquire_lock >/dev/null && type is_agent_mode >/dev/null && type extract_conv_id >/dev/null && type state_dir >/dev/null && echo "ok"' "$PACK/shared/hooks" 2>/dev/null || echo "fail")"
run_test "lib/common.sh functions available (incl. conv_id + state_dir)" "ok" "$RESULT"

rm -rf "$PACK/state"; mkdir -p "$PACK/state"
echo '{"tool_name":"Write","tool_input":{"file_path":"docs/security.md","content":"# blocks rm -rf / literally\n"}}' | bash "$PACK/shared/hooks/pre_tool_use.sh" | jq -r '.action // "allow"' > /tmp/_exec_test_out 2>/dev/null || true
