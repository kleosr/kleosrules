...
RESULT="$(cd "$FAKE_C" && printf '%s\n' "$PAYLOAD" | HOME="$FAKE_HOME" bash "$FAKE_C/hooks/session_start.sh" | jq -r --arg m "$MARKER" --arg base "$WS_BASE" '.additional_context | (test("NOW") and test($base) and (test($m)|not))')"
...
PAYLOAD="$(jq -n --arg wr "$WS" '{hook_event_name:"sessionStart",composer_mode:"agent",workspace_roots:[$wr]}')"
RESULT="$(cd "$FAKE_C" && printf '%s\n' "$PAYLOAD" | HOME="$FAKE_HOME" bash "$FAKE_C/hooks/session_start.sh" | jq -r '.additional_context | (test("NOW") and (test("KEEP_ME_NOW")|not))')"
...
run_test "session_start points at NOW.md without dumping Now body" "true" "$RESULT"
...
PAYLOAD="$(jq -n --arg wr "$WS" '{hook_event_name:"sessionStart",composer_mode:"agent",workspace_roots:[$wr]}')"
RESULT="$(cd "$FAKE_C" && printf '%s\n' "$PAYLOAD" | HOME="$FAKE_HOME" bash "$FAKE_C/hooks/session_start.sh" | jq -r '.additional_context | (test("NOW") and (test("VERIFY_MARKER")|not))')"
run_test "session_start points at NOW.md when headings are nonstandard" "true" "$RESULT"
rm -rf "$FAKE_HOME" "$WS"
