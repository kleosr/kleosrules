...
cat "$PACK/tests/fixtures/subagentStop.json" | bash "$PACK/MacOS/hooks/subagent_stop.sh" >/dev/null 2>&1 || true
...
RESULT="$(echo '{"file_path":"/home/user/.env","hook_event_name":"beforeReadFile"}' | bash "$PACK/MacOS/hooks/before_read_file.sh" | jq -r '.action // .permission // "allow"')"
run_test "before_read_file blocks .env from model context" "deny" "$RESULT"

RESULT="$(echo '{"file_path":"/home/user/src/app.ts","hook_event_name":"beforeReadFile"}' | bash "$PACK/MacOS/hooks/before_read_file.sh" | jq -r 'if . == {} then "allow" else (.action // .permission // "allow") end')"
run_test "before_read_file allows normal source" "allow" "$RESULT"

rm -rf "$PACK/state"
mkdir -p "$PACK/state"
RESULT="$(echo '{"command":"npm test","exit_code":0,"conversation_id":"conv-shell-001"}' | bash "$PACK/MacOS/hooks/after_shell.sh" | jq -r 'if . == {} then "quiet" else "noisy" end')"
