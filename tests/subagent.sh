...
cat "$PACK/tests/fixtures/subagentStop.json" | bash "$PACK/shared/hooks/subagent_stop.sh" >/dev/null 2>&1 || true
...
SHELL_LOG_LINE="$(grep -cE 'SHELL.*duration=42' "$PACK/state/conv-shell-001/session.log" 2>/dev/null || echo 0)"
