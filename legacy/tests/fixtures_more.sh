...
run_test "paste binds approval to effective destination" "1" "$(grep -c 'effective destination' "$PACK/shared/rules/USER-RULES.paste.txt" | tr -d ' ')"
run_test "paste treats prior context as continuity evidence" "1" "$(grep -c 'continuity evidence' "$PACK/shared/rules/USER-RULES.paste.txt" | tr -d ' ')"
