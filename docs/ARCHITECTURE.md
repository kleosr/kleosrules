...
- **Registration (single GLOBAL layer):** hooks register only in `~/.cursor/hooks.json` (generated by `fleet_sync.sh`). Per-repo `.cursor/hooks.json` is removed on sync — user-level and repo-level hooks both fire when both exist, doubling every `additionalContext` injection per prompt (measured 2026-08-10: DEBERES block arrived 2×/turn). Hooks spawn with cwd = workspace root, so `resolve_root` (lib/common.sh) keeps HANDOFF/state per-project under the global layer.
