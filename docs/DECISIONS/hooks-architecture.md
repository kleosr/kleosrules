...
**2026-09-03 — engineering-rules audit.** Hooks fail safe on unreadable input (`before_shell` → `ask`, `before_read_file` → `deny`, missing `secret_paths.ere` → `deny`). Source-write deny matches quoted and backslash-escaped paths; the `elif` chain became a pattern table. `rm -r` deny scoped to root/home/system/`.`/`..` so `rm -rf /tmp/x` no longer blocks. `session_start.sh` stopped writing `state/<conv>/mode` (nothing read it; it littered user workspaces). `fleet_sync.sh uninstall` + `scripts/uninstall.sh` + `Windows/uninstall.ps1` added. Removed: `lib/shell_fleet.sh` (dead bypass), nested `AGENTS.md` adapters (Claude Code `@import` syntax with no Claude consumer), `tests/conversation_state.sh`. Full record: `docs/engineering-rules-audit.md`, `docs/engineering-rules-decision.md`.
