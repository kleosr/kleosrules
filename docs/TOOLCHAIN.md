...
Expect: `continue` (submit), `additional_context` path only (sessionStart), `permission` (shell/read), `{}` or `followup_message` (stop; advisory). Submit and shell `failClosed` true. Read `failClosed` true. `stop.loop_limit` 1.
...
Uninstall: `bash scripts/uninstall.sh` (owned hook commands and pack files only; preserves other `hooks.json` keys). Caps: `docs/token-budget.md`. Roofs: `docs/quality-roofs-audit.md`.
