...
|---|---|---|---|---|---|
| GROUND | smallest correct repo/dir/task/state context | `session_start.sh` → `additional_context` (path to NOW.md); Cursor rules engine loads 7 alwaysApply `.mdc` once; glob `.mdc` on path match; root `AGENTS.md` | hook (state) + rule (law) | session start / on Read | no NOW.md → `{}`; token blob → `{}`; plan mode → `{}` |
...
| PLAN | smallest coherent change + tests; risk without speculative architecture | paste / `agent.mdc` (open the files you will change; one or two sentences: outcome, files, proof) · `ponytail.mdc` ladder · `complexity.mdc` cap | rule | before Write | judgment; not hook-enforced (no `preToolUse` by decision) |
...
| repo with `NOW.md` | path to that file (Read on demand) | `session_start.sh` |
