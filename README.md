...
2. **Inject (Layer 2)** — `session_start.sh` adds HANDOFF + short DEBERES. `before_submit_prompt.sh` classifies route and may JOB CARD nudge. `post_tool_use.sh` injects a SCORECARD after dirty writes. Never mutates the user prompt.
