...
- file_loc_legacy_emergency=700 (emergency subatomic split; reducing extract allowed)
...
- beforeSubmitPrompt → continue (secret may continue:false; FILE_MAP/culture nudge = continue:true + user_message)
- preCompact → user_message (re-Read HANDOFF/INTENT)
- stop → followup_message (incl. one soft TOOLCHAIN evidence nudge + culture; then fail open)
- quiet → {}

Scripts
sessionStart→session_start.sh ; beforeSubmitPrompt→before_submit_prompt.sh ; stop→stop_gate.sh (transcript_path) ; preToolUse Write|StrReplace→lean_gate+pre_tool_use ; beforeShellExecution→before_shell.sh ; beforeReadFile+beforeTabFileRead→before_read_file.sh ; beforeMCPExecution→before_mcp.sh ; preCompact→pre_compact.sh
...
Hard stops: never updated_input ; never Rust/Python gate ; event hooks ≤80 LOC ; Cursor tool names Write|StrReplace|Shell only
