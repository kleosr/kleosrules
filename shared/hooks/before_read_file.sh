...
case "$FILE_PATH" in
  */.env|*/.env.*|*/.aws/credentials|*/.ssh/id_*|*/.npmrc|*/.pypirc|*/.git-credentials|*/credentials.json|*/secrets.json)
    jq -n --arg m "AUTONOMY BLOCK: reading sensitive file '$FILE_PATH' blocked to protect secrets from model context. Read it yourself if needed." \
      '{permission:"deny", user_message:$m}'
    exit 0
    ;;
esac
emit_quiet
